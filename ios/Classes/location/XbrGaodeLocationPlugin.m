...
- (void)handlePlugin:(NSString *)pluginKey location:(CLLocation *)location reGeocode:(AMapLocationReGeocode *)reGeocode error:(NSError *)error{
    if (!pluginKey || ![[AMapFlutterStreamManager sharedInstance] streamHandler].eventSink)  return;
    ...
    if (location) {
        ...
        if (reGeocode) {
            if (reGeocode.country) [dic setValue:reGeocode.country forKey:@"country"];
            if (reGeocode.province) [dic setValue:reGeocode.province forKey:@"province"];
            if (reGeocode.city) [dic setValue:reGeocode.city forKey:@"city"];
            if (reGeocode.district) [dic setValue:reGeocode.district forKey:@"district"];
            if (reGeocode.street) [dic setValue:reGeocode.street forKey:@"street"];
            if (reGeocode.number) [dic setValue:reGeocode.number forKey:@"streetNumber"];
            if (reGeocode.citycode) [dic setValue:reGeocode.citycode forKey:@"cityCode"];
            if (reGeocode.adcode)  [dic setValue:reGeocode.adcode forKey:@"adCode"];
            if (reGeocode.description)  [dic setValue:reGeocode.formattedAddress forKey:@"description"];
            if (reGeocode.formattedAddress.length)  [dic setObject:reGeocode.formattedAddress forKey:@"address"];
        }
    } else {
