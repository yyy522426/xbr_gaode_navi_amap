...
const VIEW_TYPE = 'com.yilin.xbr.amap';
...
class MethodChannelAMapFlutterMap implements AMapFlutterPlatform {
  ...
  Future<void> init(int mapId) {
    ...
    if (channel == null) {
      channel = MethodChannel('xbr_amap_$mapId');
    ...
    switch (call.method) {
      case 'location#changed':
        try {
          ...
          debugPrint("location#changed error=======>" + e.toString());
        }
        break;

      case 'camera#onMove':
        try {
          _mapEventStreamController.add(CameraPositionMoveEvent(mapId, CameraPosition.fromMap(call.arguments['position'])!));
        } catch (e) {
          debugPrint("camera#onMove error===>" + e.toString());
        }
        break;
      case 'camera#onMoveEnd':
        try {
          _mapEventStreamController.add(CameraPositionMoveEndEvent(mapId, CameraPosition.fromMap(call.arguments['position'])!));
        } catch (e) {
          debugPrint("camera#onMoveEnd error===>" + e.toString());
        }
        break;
      case 'map#onTap':
      ...
      case 'marker#onDragEnd':
        _mapEventStreamController.add(MarkerDragEndEvent(mapId, LatLng.fromJson(call.arguments['position'])!, call.arguments['markerId']));
        break;
      case 'polyline#onTap':
      ...
      case 'map#onPoiTouched':
        try {
          ...
          debugPrint('map#onPoiTouched error===>' + e.toString());
        }
        break;
    }
  }
  ...
  Future<void> moveCamera(
    ...
    return channel(mapId)
        .invokeMethod<void>('camera#move', <String, dynamic>{'cameraUpdate': cameraUpdate.toJson(), 'animated': animated, 'duration': duration});
  }
